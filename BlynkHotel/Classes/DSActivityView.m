...
//  JSONDemo
//
//  Created by Brijesh J on 03/01/11.
...
- (DSActivityView *)initForView:(UIView *)addToView withLabel:(NSString *)labelText width:(NSUInteger)labelWidth;
{
    ...
    NSLog(@"Retain count before subview:%d",self.retainCount);
	[addToView addSubview:self];
    NSLog(@"Retain count after subview:%d",self.retainCount);
...
- (UIView *)borderView;
{
    if (!_borderView)
    {
        [super borderView];
			...
			UIImageView *img = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"loader.png"]];
            img.backgroundColor = [UIColor clearColor];
            
			[_borderView addSubview:img];
...
- (void)layoutSubviews;
{
	...
	//if ([WorldOfAngelAppDelegate isDeviceAniPad]) {
    ...
    borderFrame.origin.x = floor(0.4 * (self.frame.size.width - borderFrame.size.width));
    borderFrame.origin.y = floor(0.5 * (self.frame.size.height - borderFrame.size.height));
    self.borderView.frame = borderFrame;
	}
	*/
    ...
    CGRect indicatorFrame = self.activityIndicator.frame;
	indicatorFrame.origin.x = 0.80 * (borderFrame.size.width - indicatorFrame.size.width);
	indicatorFrame.origin.y = 40.0;
   // self.activityIndicator.backgroundColor = [UIColor clearColor];
    self.activityIndicator.frame = indicatorFrame;
    
    // Calculate the position of the label: horizontally centered and near the bottom of the border view:
	CGRect labelFrame = self.activityLabel.frame;
    labelFrame.origin.x = floor(1.20 * (borderFrame.size.width - labelFrame.size.width));
	labelFrame.origin.y = borderFrame.size.height - labelFrame.size.height;
    self.activityLabel.frame = labelFrame;
}
