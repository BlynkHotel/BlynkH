...
@implementation LaundryView

@synthesize mainDataArray,menuId,menuTitle;
@synthesize menuName;
...
- (void) touchesBegan:(NSSet*)touches withEvent:(UIEvent*)event
{
	...
	if (aTouch.tapCount == 1) {
		...
		[aWeatherViewController removeFromSuperview];
...
- (void)menuView
{
    ...
    */
    ...
    appDelegate.serviceIdArray1 = [response1 valueForKey:@"services_id"];
    appDelegate.nameArray1 = [response1 valueForKey:@"services_name"];
    appDelegate.rateArray1 = [response1 valueForKey:@"rate"];
    appDelegate.sortDetailArray1= [response1 valueForKey:@"short_desc"];
    appDelegate.longDetailArray1 = [response1 valueForKey:@"long_desc"];
    appDelegate.imageArray1 = [response1 valueForKey:@"image"];
    
    [appDelegate.quantityArray1 removeAllObjects];
    
    for (int q = 0; q < [appDelegate.nameArray1 count]; q++) {
        [appDelegate.quantityArray1 addObject:[NSNumber numberWithInt:0]];
    }
    ...
    if (appDelegate.isGuest == YES && appDelegate.isActive == YES) {
        ...
        if ([sIdArray count]>0) {
            ...
            for (int b = 0; b < [appDelegate.serviceIdArray1 count]; b++) {
                
                for (int h = 0; h < [sIdArray count]; h++) {
                    
                    if ([[appDelegate.serviceIdArray1 objectAtIndex:b]intValue]==[[sIdArray objectAtIndex:h]intValue]) {
                        
                        int old = [[qArray objectAtIndex:h] intValue];
                        [appDelegate.quantityArray1 replaceObjectAtIndex:b withObject:[NSNumber numberWithInt:old]];
                    }
            ...
            for (int q = 0; q < [appDelegate.serviceIdArray1 count]; q++) {
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    [appDelegate.lastMenuTitles setValue:self.menuName forKey:@"laundrymenutitle"];
    [myTimer invalidate];
...
-(void)treeView
{
    if ([appDelegate.laundryMenuArray count]==0)
    {
        NSMutableDictionary *response1 = [BKAPIClient rootMenuitem:@"7"];
        //    self.mainDataArray  = [response1 valueForKey:@"services_name"];
        appDelegate.laundryMenuArray = [response1 valueForKey:@"services_name"];
    }
    
    self.menuName = [NSString stringWithFormat:@"%@",[appDelegate.laundryMenuArray objectAtIndex:0]];
    lblTitle.text = [NSString stringWithFormat:@"%@",[appDelegate.laundryMenuArray objectAtIndex:0]];
    BOOL shouldLoadMenuView = true;
    if(appDelegate.serviceIdArray1 != nil && appDelegate.serviceIdArray1.count > 0
       && 
       appDelegate.nameArray1 != nil && appDelegate.nameArray1.count > 0
       && 
       appDelegate.rateArray1 != nil && appDelegate.rateArray1.count > 0
       &&
       appDelegate.sortDetailArray1 != nil && appDelegate.sortDetailArray1.count > 0
       && 
       appDelegate.longDetailArray1 != nil && appDelegate.longDetailArray1.count > 0
       && 
       appDelegate.imageArray1 != nil && appDelegate.imageArray1.count > 0
       && 
       appDelegate.quantityArray1 != nil && appDelegate.quantityArray1.count > 0
       )
    {
    ...
    else
    {
        ...
        if(appDelegate.lastMenuTitles != nil)
        {
            self.menuName =  [appDelegate.lastMenuTitles valueForKey:@"laundrymenutitle"];
            lblTitle.text = [NSString stringWithFormat:@"%@",menuName];
        }
        [menuTableView reloadData];
    } 
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    {
        case 0:
            row = [appDelegate.laundryMenuArray count];
            break;
        case 1: 
            row = [appDelegate.nameArray1 count];
            break;
        case 2:
...
//            cell = [[[UITableViewCell alloc] initWithFrame:CGRectZero reuseIdentifier:CellIdentifier] autorelease];
        ...
        cell.textLabel.text = [appDelegate.laundryMenuArray objectAtIndex:indexPath.row];
...
//            cell = [[[CustomCell alloc] initWithFrame:CGRectZero reuseIdentifier:CellIdentifier] autorelease];
        ...
        [itemBtn setTitle: [[appDelegate.quantityArray1 objectAtIndex:[indexPath row]] stringValue] forState:UIControlStateNormal];
        [itemBtn setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        
        [cell setSelectionStyle:UITableViewCellSelectionStyleNone];
        cell.primaryLabel.text = [appDelegate.nameArray1 objectAtIndex:indexPath.row];
        cell.secondaryLabel.text = [appDelegate.sortDetailArray1 objectAtIndex:indexPath.row];
        cell.priceLabel.text = [NSString stringWithFormat:@"%@%@",appDelegate.currency,[appDelegate.rateArray1 objectAtIndex:indexPath.row]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == aTableView) {
        ...
        self.menuName = [NSString stringWithFormat:@"%@",[appDelegate.laundryMenuArray objectAtIndex:indexPath.row]];
        //[appDelegate.quantityArray1 removeAllObjects];
        [self menuView]; 
//        [menuTableView reloadData];
...
- (IBAction)plusAction:(id)sender{
    ...
    int old = [[appDelegate.quantityArray1 objectAtIndex:correctIndex] intValue];
    [appDelegate.quantityArray1 replaceObjectAtIndex:correctIndex withObject:[NSNumber numberWithInt:old + 1]];
    
    UITableViewCell *cell=(UITableViewCell*)[[sender superview] superview];
    UITableView *table=(UITableView*)[cell superview];
	NSIndexPath *path=[table indexPathForCell:cell];
    
    NSString *serviceID = [appDelegate.serviceIdArray1 objectAtIndex:path.row];
    NSString *qnty = [appDelegate.quantityArray1 objectAtIndex:path.row];
    
    
    NSMutableDictionary *response = [BKAPIClient addtoCartItem:serviceID guestId:appDelegate.appLoginId quantity:qnty menuId:@"7"];
    appDelegate.totalArray1 = [response valueForKey:@"Total"];  
    lblTotal.text = [NSString stringWithFormat:@"Total:%@%@",appDelegate.currency,[appDelegate.totalArray1 objectAtIndex:0]];
...
//    loginView.backgroundColor = [UIColor clearColor];
    ...
    NSMutableDictionary *response = [BKAPIClient loadDisclaimer:@"7"];
...
- (void)viewCartService{
    ...
    if (appDelegate.isGuest == YES && appDelegate.isActive == YES) {
        ...
        if ([appDelegate.totalArray1 count]>0) {
            
            lblTotal1.text =  [NSString stringWithFormat:@"%@%@",appDelegate.currency,[appDelegate.totalArray1 objectAtIndex:0]];
        }
...
- (void)displayDetailView{
    ...
    lbltitle.text = [appDelegate.nameArray1 objectAtIndex:value];
    mainTitle.text = [appDelegate.sortDetailArray1 objectAtIndex:value];
    txtDetail.text = [appDelegate.longDetailArray1 objectAtIndex:value];
    lblPrice.text =  [NSString stringWithFormat:@"%@%@",appDelegate.currency,[appDelegate.rateArray1 objectAtIndex:value]];
    
    NSURL *url = [NSURL URLWithString:[appDelegate.imageArray1 objectAtIndex:value]];  
    ...
    [itemBtn1 setTitle: [[appDelegate.quantityArray1 objectAtIndex:value] stringValue] forState:UIControlStateNormal];
...
-(IBAction)incremantAction2:(id)sender{
    ...
    if (appDelegate.isGuest == YES && appDelegate.isActive == YES) {
        ...
        NSString *serviceID = [appDelegate.serviceIdArray1 objectAtIndex:correctIndex];
        NSString *qnty = [appDelegate.quantityArray1 objectAtIndex:correctIndex];
        
        NSMutableDictionary *response = [BKAPIClient addtoCartItem:serviceID guestId:appDelegate.appLoginId quantity:qnty menuId:@"7"];
        appDelegate.totalArray1 = [response valueForKey:@"Total"]; 
        
        [itemBtn1 setTitle: [[appDelegate.quantityArray1 objectAtIndex:correctIndex] stringValue] forState:UIControlStateNormal];
