...
- (void)menuView
{
    ...
    */
    ...
    appDelegate.serviceIdArray2 = [response1 valueForKey:@"services_id"];
    appDelegate.nameArray2 = [response1 valueForKey:@"services_name"];
    appDelegate.rateArray2 = [response1 valueForKey:@"rate"];
    appDelegate.sortDetailArray2= [response1 valueForKey:@"short_desc"];
    appDelegate.longDetailArray2 = [response1 valueForKey:@"long_desc"];
    appDelegate.imageArray2 = [response1 valueForKey:@"image"];
    
    [appDelegate.quantityArray2 removeAllObjects];
    
    for (int q = 0; q < [appDelegate.nameArray2 count]; q++) {
        [appDelegate.quantityArray2 addObject:[NSNumber numberWithInt:0]];
    }
    ...
    if (appDelegate.isGuest == YES && appDelegate.isActive == YES) {
        ...
        if ([sIdArray count]>0) {
            ...
            for (int b = 0; b < [appDelegate.serviceIdArray2 count]; b++) {
                
                for (int h = 0; h < [sIdArray count]; h++) {
                    
                    if ([[appDelegate.serviceIdArray2 objectAtIndex:b]intValue]==[[sIdArray objectAtIndex:h]intValue]) {
                        
                        int old = [[qArray objectAtIndex:h] intValue];
                        [appDelegate.quantityArray2 replaceObjectAtIndex:b withObject:[NSNumber numberWithInt:old]];
                    }
            ...
            for (int q = 0; q < [appDelegate.serviceIdArray2 count]; q++) {
...
- (void)viewWillDisappear:(BOOL)animated{   
    ...
    [appDelegate.lastMenuTitles setValue:self.menuName forKey:@"shoppingmenutitle"];
    [myTimer invalidate];
...
{
    //[NSThread detachNewThreadSelector:@selector(startAct) toTarget:self withObject:nil];
    if ([appDelegate.shoppingMenuArray count]==0)
    {
        NSMutableDictionary *response1 = [BKAPIClient rootMenuitem:@"8"];
        appDelegate.shoppingMenuArray  = [response1 valueForKey:@"services_name"];
    }
	
    self.menuName = [NSString stringWithFormat:@"%@",[appDelegate.shoppingMenuArray objectAtIndex:0]];
    lblTitle.text = [NSString stringWithFormat:@"%@",[appDelegate.shoppingMenuArray objectAtIndex:0]];
    
    BOOL shouldLoadMenuView = true;
    if(appDelegate.serviceIdArray2 != nil && appDelegate.serviceIdArray2.count > 0
       && 
       appDelegate.nameArray2 != nil && appDelegate.nameArray2.count > 0
       && 
       appDelegate.rateArray2 != nil && appDelegate.rateArray2.count > 0
       &&
       appDelegate.sortDetailArray2 != nil && appDelegate.sortDetailArray2.count > 0
       && 
       appDelegate.longDetailArray2 != nil && appDelegate.longDetailArray2.count > 0
       && 
       appDelegate.imageArray2 != nil && appDelegate.imageArray2.count > 0
       && 
       appDelegate.quantityArray2 != nil && appDelegate.quantityArray2.count > 0
       )
    {
    ...
    else
    {
        ...
        if(appDelegate.lastMenuTitles != nil)
        {
            self.menuName =  [appDelegate.lastMenuTitles valueForKey:@"shoppingmenutitle"];
            lblTitle.text = [NSString stringWithFormat:@"%@",menuName];
        }
        [menuTableView reloadData];
    } 
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    {
        case 0:
            row = [appDelegate.shoppingMenuArray count];
            break;
        case 1: 
            row = [appDelegate.nameArray2 count];
            break;
        case 2:
...
//            cell = [[[UITableViewCell alloc] initWithFrame:CGRectZero reuseIdentifier:CellIdentifier] autorelease];
        ...
        cell.textLabel.text = [appDelegate.shoppingMenuArray objectAtIndex:indexPath.row];
...
//            cell = [[[CustomCell alloc] initWithFrame:CGRectZero reuseIdentifier:CellIdentifier] autorelease];
        ...
        [itemBtn setTitle: [[appDelegate.quantityArray2 objectAtIndex:[indexPath row]] stringValue] forState:UIControlStateNormal];
        [itemBtn setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        
        [cell setSelectionStyle:UITableViewCellSelectionStyleNone];
        cell.primaryLabel.text = [appDelegate.nameArray2 objectAtIndex:indexPath.row];
        cell.secondaryLabel.text = [appDelegate.sortDetailArray2 objectAtIndex:indexPath.row];
        cell.priceLabel.text = [NSString stringWithFormat:@"%@%@",appDelegate.currency,[appDelegate.rateArray2 objectAtIndex:indexPath.row]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == aTableView) {
        ...
        menuView.hidden = NO;
        self.menuName = [NSString stringWithFormat:@"%@",[appDelegate.shoppingMenuArray objectAtIndex:indexPath.row]];
        //[appDelegate.quantityArray2 removeAllObjects];
        [self menuView]; 
//        [menuTableView reloadData];
...
- (IBAction)plusAction:(id)sender{
    ...
    if (appDelegate.isGuest == YES && appDelegate.isActive == YES) {
        ...
        int old = [[appDelegate.quantityArray2 objectAtIndex:correctIndex] intValue];
        [appDelegate.quantityArray2 replaceObjectAtIndex:correctIndex withObject:[NSNumber numberWithInt:old + 1]];
        
        UITableViewCell *cell=(UITableViewCell*)[[sender superview] superview];
        UITableView *table=(UITableView*)[cell superview];
        NSIndexPath *path=[table indexPathForCell:cell];
        
        NSString *serviceID = [appDelegate.serviceIdArray2 objectAtIndex:path.row];
        NSString *qnty = [appDelegate.quantityArray2 objectAtIndex:path.row];
        
        NSMutableDictionary *response = [BKAPIClient addtoCartItem:serviceID guestId:appDelegate.appLoginId quantity:qnty menuId:@"8"];
        
        appDelegate.totalArray2 = [response valueForKey:@"Total"];  
        lblTotal.text = [NSString stringWithFormat:@"Total:%@%@",appDelegate.currency,[appDelegate.totalArray2 objectAtIndex:0]];
...
//    loginView.backgroundColor = [UIColor clearColor];
    ...
    NSMutableDictionary *response = [BKAPIClient loadDisclaimer:@"8"];
...
- (void)viewCartService{
    ...
    if (appDelegate.isGuest == YES && appDelegate.isActive == YES) {
        ...
        if ([appDelegate.totalArray2 count]>0) {
            
            lblTotal1.text =  [NSString stringWithFormat:@"%@%@",appDelegate.currency,[appDelegate.totalArray2 objectAtIndex:0]];
        }
    }
}
...
-(IBAction)checkout:(id)sender{
    ...
    if (appDelegate.isGuest == YES && appDelegate.isActive == YES) {
        ...
        NSLog(@"=%@",message);
       // NSLog([NSString stringWithString:message]);
...
- (void)displayDetailView{
    ...
    lbltitle.text = [appDelegate.nameArray2 objectAtIndex:value];
    mainTitle.text = [appDelegate.sortDetailArray2 objectAtIndex:value];
    txtDetail.text = [appDelegate.longDetailArray2 objectAtIndex:value];
    lblPrice.text =  [NSString stringWithFormat:@"%@%@",appDelegate.currency,[appDelegate.rateArray2 objectAtIndex:value]];
    
    NSURL *url = [NSURL URLWithString:[appDelegate.imageArray2 objectAtIndex:value]];  
    NSData *data = [NSData dataWithContentsOfURL:url];  
    ...
    [itemBtn1 setTitle: [[appDelegate.quantityArray2 objectAtIndex:value] stringValue] forState:UIControlStateNormal];
...
-(IBAction)incremantAction2:(id)sender{
    ...
    if (appDelegate.isGuest == YES && appDelegate.isActive == YES) {
        ...
        NSString *serviceID = [appDelegate.serviceIdArray2 objectAtIndex:correctIndex];
        NSString *qnty = [appDelegate.quantityArray2 objectAtIndex:correctIndex];
        
        NSMutableDictionary *response = [BKAPIClient addtoCartItem:serviceID guestId:appDelegate.appLoginId quantity:qnty menuId:@"8"];
        appDelegate.totalArray2 = [response valueForKey:@"Total"]; 
        
        [itemBtn1 setTitle: [[appDelegate.quantityArray2 objectAtIndex:correctIndex] stringValue] forState:UIControlStateNormal];
